...
def verify_problem():
    ...
    if not (isinstance(problem['time_limit'], float) or isinstance(problem['time_limit'], int)) or problem['time_limit'] < 0.5:
        error(f"time_limit should be a number greater or equal to 0.5")
...
def verify():
    ...
    if not Verification.problem is None:
